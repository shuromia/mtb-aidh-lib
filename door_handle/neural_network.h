...
/**
 * \brief       Run inference on the color classification model
 * \details     This prepares the color values read by the ADC for use with the model and runs
 *              inference on the model. It returns an index of the color LUT (look-up table)
 *              which has the highest probability and is therefore the best prediction.
 *
 * \param[in]   cap_val       Struct containing color values read from the Capsense
 * \param[out]  index         Pointer for writing the identified color index to
